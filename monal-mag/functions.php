...
// Add setting for heart button visibility
...
// Add setting for ring button visibility
$wp_customize->add_setting( 'ring_button_visibility', array(
	'default' => 'show',
	'sanitize_callback' => 'sanitize_text_field',
) );

// Add control for heart button visibility
...
// Add control for ring button visibility
$wp_customize->add_control( 'ring_button_visibility', array(
	'label' => 'Ring Button Visibility',
	'section' => 'button_settings',
	'settings' => 'ring_button_visibility',
	'type' => 'radio',
	'choices' => array(
		'show' => 'Show',
		'hide' => 'Hide',
...
//         'name' => __('Footer Widgets', 'custom-theme'),
//         'id' => 'footer-widgets',
...
//     echo '<style>';
//     echo '.site-footer { background-color: ' . $footer_bg_color . '; }';
//     echo '</style>';
// }
// add_action('wp_head', 'custom_theme_footer_styles');
// functions.php
// functions.php
function custom_theme_apply_footer_style() {
    $footer_style = get_theme_mod('footer_style', 'style-default');

    echo '<style>';
    switch ($footer_style) {
        case 'style-dark':
            echo '.site-footer { background-color: #333; color: #fff; }';
            break;
        case 'style-light':
            echo '.site-footer { background-color: #f5f5f5; }';
            break;
        case 'style-red':
            echo '.site-footer { background-color: #ff0000; color: #fff; }';
            break;
        case 'style-blue':
            echo '.site-footer { background-color: #0000ff; color: #fff; }';
            break;
...
add_action('wp_head', 'custom_theme_apply_footer_style');
...
function child_theme_enqueue_scripts() {
    wp_enqueue_script('customizer-script', get_stylesheet_directory_uri() . '/customizer.js', array('jquery', 'customize-preview'), null, true);
}
...
function child_theme_customizer_settings($wp_customize) {
    ...
    $wp_customize->add_section('hero_section', array(
        'title' => __('Hero Section', 'child-theme'),
        'priority' => 30,
    ));

    // Add setting
    $wp_customize->add_setting('hero_text', array(
        'default' => 'Default Hero Text',
        'sanitize_callback' => 'sanitize_text_field',
    ));

    // Add control
    $wp_customize->add_control('hero_text', array(
        'label' => __('Hero Text', 'child-theme'),
        'section' => 'hero_section',
        'type' => 'text',
    ));
	 // Add setting
	 $wp_customize->add_setting('hero_content', array(
        'default' => 'Default Hero content',
        'sanitize_callback' => 'sanitize_text_field',
    ));

    // Add control
    $wp_customize->add_control('hero_content', array(
        'label' => __('Hero content', 'child-theme'),
        'section' => 'hero_section',
        'type' => 'text',
    ));
    ...
    $wp_customize->add_setting('hero_image', array(
        'default' => '',
        'sanitize_callback' => 'esc_url_raw', // Use esc_url_raw to sanitize image URL
    ));

    // Add control for hero image
    $wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'hero_image', array(
        'label' => __('Hero Image', 'child-theme'),
        'section' => 'hero_section',
        'settings' => 'hero_image',
    )));
    ...
    $wp_customize->add_control('hero_button_text', array(
        'label' => __('Hero Button Text', 'child-theme'),
        'section' => 'hero_section',
        'type' => 'text',
    ));
    ...
    $wp_customize->add_setting('hero_button_url', array(
        'default' => '#',
        'sanitize_callback' => 'esc_url_raw', // Use esc_url_raw to sanitize URL
    ));

    // Add control for hero button URL
    $wp_customize->add_control('hero_button_url', array(
        'label' => __('Hero Button URL', 'child-theme'),
        'section' => 'hero_section',
        'type' => 'url',
    ));
